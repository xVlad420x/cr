...
class User:
    class PlayerInput:
        def __init__(self):
            ...
            self.grid_count = 0
            self.failspawn_destroy_count = 0
            self.tools_use_csv = False
    ...
    class CodeRaidTools:
        ...
        def get_path(self,doorid,spawnid):
            for path in self.path_list:
                if (path.door.door_id == doorid) and (path.spawn.spawn_id == spawnid):
                    return path
            return None
    ...
    def type(self,string1):
        ...
        for character in string1:
            time.sleep(self.player_input.console_delay/8)
        ...
        if os.path.exists(config_path):
            with open(config_path, "r") as file:
                for line in file:
                    ...
                    elif ("Completed_Codes" in myline):
                        self.standard_info.current_code_count = int(myline[(myline.index("=") + 1):len(myline)].strip())
                    elif ("Copy" in myline):
                    ...
                    elif ("Failed_Spawns_Means_Destroyed" in myline):
                        self.player_input.failspawn_destroy_count = int(myline[(myline.index("=") + 1):len(myline)].strip())
                    elif ("Tools_From_CSV" in myline):
                        self.player_input.tools_use_csv = str_to_bool(myline[(myline.index("=") + 1):len(myline)].strip())
            ...
            #print("Current Code Count :" + str(self.standard_info.current_code_count))
        else:
    ...
    def get_to_door_and_face_lock(self, path_object: Path, departure_time: float) -> bool:
        segment_run_count = 0
        for location in path_object.locationlist:
            if(segment_run_count > 0): #If segment count is 0 then location and vision are already accurate from spawn
                self.update_location(True, False)
                self.update_vision(False, True)
            self.move_directly_to_cord(self.standard_info.map_vision, self.standard_info.map_location,
                                       location)
            segment_run_count += 1
        #We are approximately near the door
        distance = 999999
        if(segment_run_count > 0):
            self.update_location(True, False)
            self.update_vision(False, True)
        while (distance >= 0.8):
            if (distance > 4.5):
    ...
    def spawn(self, spawn_object: Spawn, current_location: Location) -> bool:
        initial_mouse_pos = list(outputmouse.position)
        ...
        self.wait(self.player_input.console_delay * 3)
        initial_position = self.standard_info.map_location
        self.update_location(True, False)
        self.update_vision(False, True)
        postspawn_position = self.standard_info.map_location
        if(initial_position.xcoord == postspawn_position.xcoord) and (initial_position.ycoord == postspawn_position.ycoord):
            #failed spawn
            spawn_object.failed_spawn_count+= 1
            if(spawn_object.failed_spawn_count >= self.player_input.failspawn_destroy_count):
                self.status.destroyed_bag = True
                spawn_object.is_destroyed = True
            outputmouse.position = (initial_mouse_pos[0], initial_mouse_pos[1])
            return False
        spawn_object.time_of_last_spawn = timestamp() - (self.player_input.console_delay * 28)
        return True
    ...
    def update_location(self,must_open_console,must_close_console):
        ...
        outputmouse.click(Button.left, 1)
        self.wait(self.player_input.console_delay * 0.5)
        outputmouse.position = (self.console_cords.input_cord[0], self.console_cords.input_cord[1])
        self.wait(self.player_input.console_delay * 1)
        outputmouse.click(Button.left, 1)
        self.wait(self.player_input.console_delay * 0.5)
    ...
    def clear_code_csvs(self):
        df = pd.read_csv(sys.argv[5])
        df = df.head(0)
        df.to_csv(sys.argv[5], index=False)
    def clear_runtime_csvs(self):
        for i in range(3, 7):
            df = pd.read_csv(sys.argv[i])
            df = df.head(0)
            df.to_csv(sys.argv[i], index=False)

    def update_code_csv(self):
        df = pd.read_csv(sys.argv[5])
        df.loc[len(df)] = ([self.codes_df.iloc[self.standard_info.current_code_count,0],self.standard_info.current_code_count,timestamp()])
        df.to_csv(sys.argv[5], index = False)

    def update_door_csv(self,door_id):
        df = pd.read_csv(sys.argv[3])
        door_vision = self.standard_info.map_vision
        door_location = self.standard_info.map_location
        df.loc[len(df)] = ([door_location.xcoord,door_location.ycoord,door_location.zcoord,
                            door_vision.xcoord, door_vision.ycoord, door_vision.zcoord,door_id
                            ])
        df.to_csv(sys.argv[3], index = False)

    def update_spawn_csv(self,is_bed,door_id,spawn_id,is_detour,detour_number):
        df = pd.read_csv(sys.argv[4])
        bag_loc = self.standard_info.map_location
        df.loc[len(df)] = ([bag_loc.xcoord, bag_loc.ycoord, bag_loc.zcoord,
                            is_bed,door_id,spawn_id,is_detour,detour_number
                            ])
        df.to_csv(sys.argv[4], index = False)

    def read_config_and_codes(self):
        self.read_config()
        self.read_codefile()
        ...
        def listentoendinitialization():
            ...
            def on_release(key):
                ...
                if key == self.player_input.killswitch:
                    ...
                    self.update_door_csv(doorid)
                    doorid+=1
                    #We need to avoid accidental duplicates
                    for door in doorlist:
                        if (door.location == door_location):
                            print("ERROR: duplicate door")
                            sys.exit(0)
                    ...
                    self.update_spawn_csv(False,doorlist[-1].door_id,spawnid,False,0)
                    spawnid+=1
                    for spawn in spawnlist:
                        if (spawn.location == bag_location):
                            print("ERROR: duplicate bag location")
                            sys.exit(0)
                    ...
                    self.update_spawn_csv(True, doorlist[-1].door_id, spawnid, False, 0)
                    print("At location: " + str(bed_location))
                    spawnid += 1
                    for spawn in spawnlist:
                        if (spawn.location == bed_location):
                            print("ERROR: duplicate bed location")
                            sys.exit(0)
                    spawnlist.append(newspawn)
                    newpath = Path(newspawn, doorlist[-1])
                    path_dict[newspawn.spawn_id] = newpath
                    ...
                    self.update_spawn_csv(False, currentpath.door.door_id, currentpath.spawn.spawn_id,
                                          True,len(currentpath.locationlist))
    ...
    def load_tools_from_csvs(self):
        doorlist = []
        spawnlist = []
        path_dict = {}
        path_list = []

        def find_door(doorid):
            for door in doorlist:
                if doorid == door.door_id:
                    return door
            print("Door not found")
            raise KeyError

        door_csv = pd.read_csv(sys.argv[3])
        for i, row in door_csv.iterrows():
            pass
            door_loc = Location(float(row["door_loc_x"]),float(row["door_loc_y"]),float(row["door_loc_z"]))
            door_vis = Vision(float(row["door_vis_x"]),float(row["door_vis_y"]),float(row["door_vis_z"]))
            door_id = int(row["door_id"])
            print(row["door_loc_x"])
            newdoor = Door(door_loc,door_vis,door_id)
            doorlist.append(newdoor)
        spawn_csv = pd.read_csv(sys.argv[4])
        spawn_csv.sort_values("detour_number",axis = 0,ascending=True,inplace=True)
        for i, row in spawn_csv.iterrows():
            loc = Location(float(row["bag_loc_x"]),float(row["bag_loc_y"]),float(row["bag_loc_z"]))
            isbed = bool(row["is_bed"])
            door_id = int(row["door_id"])
            spawn_id = int(row["spawn_id"])
            isdetour = bool(row["is_detour"])
            if not isdetour:
                newspawn = Spawn(loc, isbed, door_id, spawn_id)
                spawnlist.append(newspawn)
                targetdoor = find_door(door_id)
                newpath = Path(newspawn, targetdoor)
                path_dict[newspawn.spawn_id] = newpath
                path_list.append(newpath)
            else:
                currentpath = path_dict[spawn_id]
                currentpath.updatepath(loc)
        self.tools.update(spawnlist, doorlist, path_dict, path_list)
        self.suicide(True, True)
        self.set_zoom()
        self.get_ingame_sens()
    ...
    def coderaid(self):
        while (self.standard_info.current_code_count <= 10000):
            self.wait(0.5)
            print("Waiting for available door")
            ...
            #If we could not pick bag try again
            if not(self.spawn(optimalpath.spawn,self.standard_info.map_location)):
                continue
            #If we fail to get to the door then look for new bag
            if not (self.get_to_door_and_face_lock(optimalpath, startrun_time)):
                continue
    ...
    #Main Drivercode
    def main(self):
        self.read_config_and_codes()
        if self.player_input.tools_use_csv:
            self.clear_code_csvs()
            time.sleep(1.5)
            self.load_tools_from_csvs()
        else:
            self.clear_runtime_csvs()
            self.create_tools()
        self.wait_for_start()
...
#test.read_config()
#test.read_codefile()
#print(test.codes_df["Entries"].head(1000).sum()/test.codes_df["Entries"].sum())
